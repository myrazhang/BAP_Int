...
public class BendersStolletz extends BendersBAP {
    ...
    public void solveBAPWithStolletz(double[][] tij){
    ...
    public void addFeasibilityCut(){
        try{
            ...
            {
                for (int k= (int) this.mySystem.buffer[j] +1; k<this.upperBoundj[j]; k++)
                {
    ...
    public void setupVariables(){
        try{
            this.zjk=new IloNumVar[this.mySystem.nbStage-1][];
            for(int j=0;j<this.mySystem.nbStage-1;j++){
                this.zjk[j]=new IloNumVar[this.upperBoundj[j]];
